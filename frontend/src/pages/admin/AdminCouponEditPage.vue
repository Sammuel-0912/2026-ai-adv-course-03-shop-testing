...
const coupon = computed(() =>
  store.coupons.find((item) => item.id === couponId.value),
)
...
async function handleSubmit(payload: CreateCouponPayload) {
  ...
  errorMessage.value = ''
  try {
    await store.updateCoupon(coupon.value.id, {
    ...
    errorMessage.value =
      error instanceof ApiError ? error.message : '更新優惠券失敗，請稍後再試'
  } finally {
  ...
  <div class="mx-auto max-w-[1280px] px-5 py-9 sm:px-8 lg:px-12 lg:py-12">
    <template v-if="coupon">
      <div class="mb-8 border-b border-[#171913] pb-7">
        ...
        <h1
          class="mt-2 font-serif text-4xl tracking-[-.03em] italic sm:text-6xl"
        >
          編輯優惠券
        </h1>
        <p class="mt-4 max-w-2xl text-sm leading-7 text-[#65685f]">
          可調整折扣條件、總使用額度與啟用狀態；代碼及已使用次數依 API
          契約保持唯讀。
        </p>
      </div>
      <CouponForm
    ...
    <div
      ...
      class="mx-auto mt-20 max-w-lg border border-[#171913] bg-[#fffdf8] p-8 text-center shadow-[8px_8px_0_#171913]"
    >
      <span class="font-serif text-6xl text-[#b1b3a9] italic">404</span>
      <h1 class="mt-3 text-xl font-semibold">找不到這張優惠券</h1>
      <p class="mt-3 text-sm text-[#74776d]">
        優惠券可能不存在，或網址中的識別碼不正確。
      </p>
      <RouterLink to="/admin/coupons" class="admin-primary-button mt-6"
        >返回優惠券列表</RouterLink
      >
    </div>
  </div>
